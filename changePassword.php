...
if(isset($_POST['passchange'])){
	...
	echo '<p class="cpass">All the fields are required</p>'; 
		...
		$sql =$con->query( "SELECT * FROM userdetails where contact_id= '".$_SESSION['contact_id']."'");
	 ...
	 if($member['password']!=$oldpass){
		...
		$query = $con->query("update userdetails set password='$newpass' where contact_id= '".$_SESSION['contact_id']."'");
		if($query){
